...
def gmean(X, y, est):
    pred = est.predict(X)
    return geometric_mean_score(y, pred, average = 'macro')
...
early = EarlyStopping(
    ...
    scorer=partial(gmean, dataset.X_validate, dataset.y_validate),
    patience = 10,
...
evaluator.append_to_file(f'Training time (seconds): {elapsed_time_training}', "time.txt")
evaluator.append_to_file(f'Testing time (seconds): {elapsed_time_testing}', "time.txt")
...
evaluator.save_evaluation_metric_results(early.scores_)
evaluator.create_evaluation_metric_results(early.scores_)
